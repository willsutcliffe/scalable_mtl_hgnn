...
class NeutralsHeteroGNN(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 dropout=0.0):
        """
        Initializes the NeutralsHeteroGNN model with encoders, message-passing blocks, and decoders.
        ...
        for b, core in enumerate(self._blocks):
            ...
            if core.node_prune == True:
                for node_type in self.node_types:
                    if node_type in ['chargedtree', 'neutrals']:
                        node_indices = core.node_indices[node_type]
                        # liste des edge_types à considérer pour chaque type
                        relevant_edge_types = [et for et in self.edge_types if node_type in et]
                        node_pruning(node_indices, latent0, node_type, relevant_edge_types, device=core.device)
